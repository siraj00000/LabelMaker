...
class AccountController {
    ...
    public async forgotPassword(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            // If the account doesn't exist, return an error
            if (!account || !userVerificationData) return next(new ErrorResponse(404, 'Account not found.'));
    ...
    public async resetPassword(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (!account || !userVerificationData) return next(new ErrorResponse(400, 'Invalid or expired reset password token.'));
    ...
    public async createStaffAccount(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
    ...
    public async getAllAccounts(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const filter: Record<string, any> = { role: { $ne: 'Super Admin' } };
    ...
    // Method for admin to unblock a user and send a new password via email
    public async unblockUserAndSendPassword(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { id } = req.params;
    ...
    public async deleteOneAccount(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { id } = req.params;
    ...
    public async deleteManyAccount(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { ids } = req.body;
            ...
            const accounts: IAccount[] = await Account.find({ _id: { $in: ids } });

            // Filter out Super Admin accounts
            const nonSuperAdminAccounts: IAccount[] = accounts.filter(account => account.role !== 'Super Admin');

            // Extract IDs of non-Super Admin accounts
            const nonSuperAdminAccountIds: string[] = nonSuperAdminAccounts.map(account => account._id.toString());
