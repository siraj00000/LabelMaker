...
import { REQUEST_HELP_FIELD_LIST } from '@/app/data/FormFieldData';
...
const ReqHelpForm = ({ dsN, link }: Props) => {
    ...
    return (
        <form method='POST' onSubmit={formik.handleSubmit} className='max-w-3xl w-full  border rounded-xl md:p-10 px-5 py-10 space-y-10 mx-auto my-10'>
            <h1 className='md:text-5xl text-2xl text-center font-medium text-black capitalize'>Request Help Form</h1>
            <fieldset className='max-full grid md:grid-cols-2 grid-cols-1 gap-5 mx-auto'>

                {REQUEST_HELP_FIELD_LIST.map((field, fieldIdx) => (
                    <TextField
                        key={fieldIdx}
                        id={field.name}
                        type={field.type}
